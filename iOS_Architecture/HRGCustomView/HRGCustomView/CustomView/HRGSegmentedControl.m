...
@interface HRGScrollView : UIScrollView

@end

@implementation HRGScrollView
...
@property (nonatomic, strong) HRGScrollView *scrollView;

@end

@implementation HRGSegmentedControl
...
- (void)commonInit {
    self.scrollView = [[HRGScrollView alloc] init];
    ...
    self.selectedTextColor = UIColorFromRGB(0x03a3d6);
    self.backgroundColor = [UIColor whiteColor];
    self.opaque = NO;
    self.selectionIndicatorColor = UIColorFromRGB(0x03a3d6);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        ...
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            CGRect textRect = CGRectMake(textXOffset,
                                         yOffset,
                                         [[self.segmentWidthsArray objectAtIndex:idx] floatValue]-imageWidth-segmentImageTextPadding-self.segmentEdgeInset.left-self.segmentEdgeInset.right,
                                         stringHeight);
            CATextLayer *titleLayer = [CATextLayer layer];
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        if (self.selectedSegmentIndex < self.sectionTitles.count) {
            NSString *title = [self.sectionTitles objectAtIndex:self.selectedSegmentIndex];
            CGFloat stringWidth = [title sizeWithAttributes:@{ NSFontAttributeName:self.font }].width;
            sectionWidth = stringWidth;
        }
    } else if (self.type == HMSegmentedControlTypeImages) {
        if (self.selectedSegmentIndex < self.sectionImages.count) {
