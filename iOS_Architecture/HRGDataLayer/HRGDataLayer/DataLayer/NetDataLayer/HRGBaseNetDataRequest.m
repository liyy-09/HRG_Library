...
-(RACSignal *) requestWithURL:(NSString *) url params:(id) params requestType:(RequestType) requestType
    ...
    return [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        if (token) {
            [session.requestSerializer setValue:token forHTTPHeaderField:model.tokenKey];
        }
        ...
        if ([[response allKeys] containsObject:model.resultKey]) {
            ...
            if (resultValue && [resultValue isEqualToString:model.successIden]) {// resultValue存在, 并表示成功时
                ...
                if (cache && isCache) {
                    ...
                    [self gainCacheDataWithSubscriber:subscriber error:nil cache:cache cacheKey:cacheKey isCache:isCache tint:message requestModel:model];
                } else {
                    // 来自缓存数据 错误了,只能提示用户 数据错误
                    NetDataReturnModel *returnModel = [[NetDataReturnModel alloc] init];
                    returnModel.type = ReturnFailure;
                    returnModel.error = requestTint ? requestTint : message;
                    [subscriber sendNext:returnModel];
                    [subscriber sendCompleted];
                }
            }
